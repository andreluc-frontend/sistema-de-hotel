...
function ReservaForm() {
  ...
  async function handleSubmit(e) {
    ...
    if (!res.ok) {
      setErro(data.error || "Erro ao realizar reserva.");
      return;
    }
    ...
    return (
      <div className="max-w-lg mx-auto mt-16 bg-white rounded-2xl shadow-lg p-8 text-center border border-gray-100">
        ...
        <p className="text-gray-600 mb-6">
          Quarto número <strong className="text-[#0071c2]">{sucesso.quarto}</strong> reservado com sucesso.
        </p>
        <div className="bg-[#eaf0fb] rounded-xl p-4 mb-6 text-left">
          <p className="text-sm text-[#003580] font-semibold mb-2">📋 Resumo da reserva</p>
        ...
        <div className="flex gap-3 justify-center">
          <Link href="/" className="px-5 py-2 border-2 border-[#003580] text-[#003580] rounded-lg font-semibold text-sm hover:bg-[#eaf0fb] transition">
            Voltar ao início
          </Link>
          <Link href="/login" className="px-5 py-2 bg-[#003580] text-white rounded-lg font-semibold text-sm hover:bg-[#00256a] transition">
            Fazer login
          </Link>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-[#0071c2] focus:ring-1 focus:ring-[#0071c2] transition"
                  required
                />
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <select
                  ...
                  className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-[#0071c2] focus:ring-1 focus:ring-[#0071c2] transition bg-white"
                  required
                >
                  <option value="">Selecione um quarto</option>
                  {quartos.map((q) => (
                    <option key={q.id} value={q.id}>
                      Quarto {q.id} — {q.nome} (R$ {q.preco}/noite)
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="md:col-span-2 mt-2">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-[#febb02] text-[#003580] font-bold py-3.5 rounded-xl text-base hover:bg-[#f0b400] transition disabled:opacity-50"
                >
                  {loading ? "Processando reserva..." : "✅ Confirmar reserva"}
                </button>
                <p className="text-xs text-gray-400 text-center mt-3">🔒 Seus dados são protegidos com criptografia</p>
              </div>
        ...
        <div className="md:col-span-1">
          {quartoSelecionado ? (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sticky top-24">
              ...
              <div className="p-5">
                <span className="text-xs bg-[#003580] text-white px-2 py-0.5 rounded font-semibold">
                ...
                <div className="flex flex-wrap gap-1 mb-4">
                  {quartoSelecionado.comodidades.map((c) => (
                    <span key={c} className="text-xs bg-[#eaf0fb] text-[#003580] px-2 py-0.5 rounded-full">{c}</span>
                  ))}
                </div>
                <div className="border-t border-gray-100 pt-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xl font-extrabold text-gray-800">
                      R$ {quartoSelecionado.preco.toLocaleString("pt-BR")}
                    </span>
                  </div>
                  <span className="bg-[#003580] text-white text-xs font-bold px-1.5 py-0.5 rounded mt-2 inline-block">
                    ⭐ {quartoSelecionado.avaliacao.toFixed(1)} Excelente
                  </span>
                </div>
              </div>
            </div>
