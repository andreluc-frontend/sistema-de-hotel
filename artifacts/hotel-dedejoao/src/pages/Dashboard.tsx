...
export default function Dashboard() {
  ...
  const [hospede, setHospede] = useState<HospedeInfo | null>(null);
  const [quartoInfo, setQuartoInfo] = useState<typeof quartos[0] | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    const dados = JSON.parse(hospedeData) as HospedeInfo;
    setHospede(dados);
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {quartoInfo.comodidades.map((c) => (
                      <span key={c} className="bg-blue-100 text-blue-700 text-xs px-3 py-1 rounded-full">{c}</span>
                    ))}
                  </div>
                </div>
